...
function generate_rating_result_html($entries, $rating_result, $show_no_result_text, $post_id) {
	...
	if ($entries != 0) {
		...
		$stars_img_url = plugins_url('img' . DIRECTORY_SEPARATOR . 'stars_sprite.png', __FILE__);
		//$stars_gray_img_url = plugins_url('img' . DIRECTORY_SEPARATOR . 'grey_stars.png', __FILE__);
		
		$html .= '<div itemscope itemtype="http://schema.org/Article" class="ratingResult">';
		
		$html .= '
		<span class="ratingStars" style="text-align: left; display: inline-block; width: 130px; height: 24px; background: url(' . $stars_img_url . ') 0 0; padding: 0px; margin: 0px;">
			<span style="display: inline-block; width: ' . $rating_result_percentage . '%; height: 24px; background: url(' . $stars_img_url . ') 0 -25px;"></span>
		</span>
		';
